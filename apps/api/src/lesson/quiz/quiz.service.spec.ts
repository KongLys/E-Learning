...
const mockPrisma = {
  ...
  quizQuestion: { create: jest.fn(), findUnique: jest.fn(), update: jest.fn(), delete: jest.fn() },
...
describe('QuizService', () => {
  ...
  describe('addQuestion', () => {
    ...
    it('throws NotFoundException when lesson not found', async () => {
      ...
      await expect(service.addQuestion('lesson-1', 'user-1', 'instructor', {} as any))
        .rejects.toThrow(NotFoundException);
    });

    it('throws ForbiddenException when not the course instructor', async () => {
      mockPrisma.lesson.findUnique.mockResolvedValue(lessonWithCourse('other-instructor'));
      await expect(service.addQuestion('lesson-1', 'user-1', 'instructor', {} as any))
        .rejects.toThrow(ForbiddenException);
    });

    it('throws NotFoundException when quiz not configured', async () => {
      mockPrisma.lesson.findUnique.mockResolvedValue(lessonWithCourse('instructor-1'));
      mockPrisma.quizLesson.findUnique.mockResolvedValue(null);
      await expect(service.addQuestion('lesson-1', 'instructor-1', 'instructor', {} as any))
        .rejects.toThrow(NotFoundException);
    });
    ...
    it('throws BadRequestException for single-choice with 2 correct answers', async () => {
      ...
      await expect(service.addQuestion('lesson-1', 'instructor-1', 'instructor', dto as any))
    ...
    it('creates question successfully for valid single-choice dto', async () => {
      ...
      const result = await service.addQuestion('lesson-1', 'instructor-1', 'instructor', dto as any);
  ...
  describe('getQuiz', () => {
    it('throws NotFoundException when quiz not found', async () => {
      mockPrisma.quizLesson.findUnique.mockResolvedValue(null);
      await expect(service.getQuiz('lesson-1', 'user-1', 'student'))
        .rejects.toThrow(NotFoundException);
    });
    ...
    it('hides isCorrect for non-instructor users', async () => {
      const quiz = {
        ...
        questions: [{
          id: 'q-1',
          options: [{ id: 'o-1', isCorrect: true }, { id: 'o-2', isCorrect: false }],
        }],
      };
      mockPrisma.quizLesson.findUnique.mockResolvedValue(quiz);
      mockPrisma.lesson.findUnique.mockResolvedValue(lessonWithCourse('instructor-1'));
      ...
      expect(result.questions[0].options.every((o) => o.isCorrect === false)).toBe(true);
    });
    ...
    it('returns isCorrect for instructor', async () => {
      ...
      const result = await service.getQuiz('lesson-1', 'instructor-1', 'instructor');
