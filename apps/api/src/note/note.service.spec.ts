...
const mockPrisma = {
  ...
  note: { create: jest.fn(), findMany: jest.fn(), findUnique: jest.fn(), update: jest.fn(), delete: jest.fn() },
};
...
describe('NoteService', () => {
  ...
  describe('createNote', () => {
    it('throws BadRequestException for video_timestamp with negative positionValue', async () => {
      const dto = { lessonId: 'lesson-1', content: 'test', positionType: PositionType.VIDEO_TIMESTAMP, positionValue: -1 };
      await expect(service.createNote('student-1', dto)).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequestException for document_page with positionValue = 0', async () => {
      const dto = { lessonId: 'lesson-1', content: 'test', positionType: PositionType.DOCUMENT_PAGE, positionValue: 0 };
      await expect(service.createNote('student-1', dto)).rejects.toThrow(BadRequestException);
    });

    it('throws NotFoundException when lesson not found', async () => {
      mockPrisma.lesson.findUnique.mockResolvedValue(null);
      const dto = { lessonId: 'bad', content: 'test', positionType: PositionType.NONE, positionValue: 0 };
      await expect(service.createNote('student-1', dto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ForbiddenException when not enrolled', async () => {
      ...
      const dto = { lessonId: 'lesson-1', content: 'test', positionType: PositionType.NONE, positionValue: 0 };
      await expect(service.createNote('student-1', dto)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('creates note successfully when enrolled', async () => {
      ...
      const dto = { lessonId: 'lesson-1', content: 'test', positionType: PositionType.VIDEO_TIMESTAMP, positionValue: 155 };
  ...
  describe('getNotesByLesson', () => {
    ...
    it('throws ForbiddenException for student-2 when not enrolled', async () => {
      mockPrisma.lesson.findUnique.mockResolvedValue(lessonWithSection);
      mockPrisma.enrollment.findFirst.mockResolvedValue(null);
      await expect(service.getNotesByLesson('student-2', 'lesson-1')).rejects.toThrow(ForbiddenException);
    });
  });

  describe('updateNote', () => {
    it('throws ForbiddenException when note belongs to different student', async () => {
      mockPrisma.note.findUnique.mockResolvedValue({ id: 'note-1', studentId: 'student-1' });
      await expect(service.updateNote('note-1', 'student-2', { content: 'new' })).rejects.toThrow(ForbiddenException);
    });
  });
});
