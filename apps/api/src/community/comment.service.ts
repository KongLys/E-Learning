...
export class CommentService {
  ...
  private async assertEnrolledOrInstructor(courseId: string, userId: string, userRole: string) {
    ...
    if (!enrollment) throw new ForbiddenException('Must be enrolled to comment');
  }

  async addComment(postId: string, authorId: string, userRole: string, dto: CreateCommentDto) {
    ...
    return this.prisma.postComment.create({
      data: { postId, authorId, body: dto.body, parentId: dto.parentId ?? null },
  ...
  async updateComment(commentId: string, userId: string, body: string) {
    ...
    return this.prisma.postComment.update({ where: { id: commentId }, data: { body } });
  }
  ...
  async deleteComment(commentId: string, userId: string, userRole: string) {
    ...
    if (!isOwner && !isInstructorOfCourse && userRole !== 'admin') throw new ForbiddenException();
    await this.prisma.postComment.update({ where: { id: commentId }, data: { status: 'deleted' } });
  ...
  async markSolution(commentId: string, userId: string) {
    ...
    if (!comment) throw new NotFoundException('Comment not found');
    if (comment.post.authorId !== userId) throw new ForbiddenException('Only post author can mark solution');
    if (comment.post.type !== 'question') throw new ForbiddenException('Only questions can have solutions');
    ...
    return this.prisma.postComment.update({ where: { id: commentId }, data: { isSolution: newValue } });
  }
}
