...
const makeQuestion = (overrides: Partial<any> = {}) => ({
  ...
  isPublic: true,
  lesson: {
    section: { course: { instructorId: 'instructor-1' }, courseId: 'course-1' },
  },
  ...overrides,
});
...
describe('QuickQuestionService', () => {
  ...
  describe('addReply', () => {
    it('throws ForbiddenException when student tries to reply', async () => {
      ...
      await expect(
        service.addReply('q-1', 'student-2', 'student', { content: 'reply' }),
      ).rejects.toThrow(ForbiddenException);
    });

    it('allows instructor to reply', async () => {
      mockPrisma.quickQuestion.findUnique.mockResolvedValue(makeQuestion());
      mockPrisma.questionReply.create.mockResolvedValue({
        id: 'reply-1',
        content: 'reply',
      });
  ...
  describe('reopenQuestion', () => {
    it('throws UnprocessableEntityException when status is closed', async () => {
      mockPrisma.quickQuestion.findUnique.mockResolvedValue(
        makeQuestion({ status: 'closed', studentId: 'student-1' }),
      );
      await expect(service.reopenQuestion('q-1', 'student-1')).rejects.toThrow(
        UnprocessableEntityException,
      );
    });
    ...
    it('throws ForbiddenException when not the owner', async () => {
      ...
      await expect(service.reopenQuestion('q-1', 'student-2')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('reopens from answered status', async () => {
      mockPrisma.quickQuestion.findUnique.mockResolvedValue(
        makeQuestion({ status: 'answered', studentId: 'student-1' }),
      );
      mockPrisma.quickQuestion.update.mockResolvedValue({
        id: 'q-1',
        status: 'pending',
  ...
  describe('closeQuestion', () => {
    it('throws NotFoundException when question not found', async () => {
      ...
      await expect(
        service.closeQuestion('q-1', 'student-1', 'student'),
      ).rejects.toThrow(NotFoundException);
    });

    it('allows owner to close', async () => {
      mockPrisma.quickQuestion.findUnique.mockResolvedValue(
        makeQuestion({ studentId: 'student-1' }),
      );
      mockPrisma.quickQuestion.update.mockResolvedValue({
        id: 'q-1',
        status: 'closed',
