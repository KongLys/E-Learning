...
describe('RagService - relevance gate', () => {
  ...
  it('điểm rerank thấp ⇒ "chưa đề cập", không citation, không gọi LLM trả lời', async () => {
  ...
  it('điểm cao + compression có nội dung ⇒ trả lời bình thường kèm citation', async () => {
    mockCohere.rerank.mockResolvedValue([
      { index: 0, relevanceScore: 0.91 },
      { index: 1, relevanceScore: 0.2 }, // dưới ngưỡng 0.3 → bị loại
    ]);
    ...
    expect(res.citations).toHaveLength(1); // chỉ chunk đạt ngưỡng
