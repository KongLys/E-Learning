...
export class LlamaParseService {
  ...
  constructor(config: ConfigService) {
    ...
    this.pollInterval = config.get<number>(
      'LLAMA_PARSE_POLL_INTERVAL_MS',
      5000,
    );
    this.pollTimeout = config.get<number>(
      'LLAMA_PARSE_POLL_TIMEOUT_MS',
      300000,
    );
    if (!this.apiKey) {
      this.logger.warn(
        'LLAMA_CLOUD_API_KEY is not set — lesson document parsing will fail at runtime',
      );
    }
  }
  ...
  async submitJob(
    ...
    if (!res.ok) {
      ...
      throw new ServiceUnavailableException(
        `LlamaParse upload failed (${res.status}): ${errBody.slice(0, 200)}`,
      );
    }
  ...
  async pollUntilDone(jobId: string): Promise<void> {
    ...
    while (Date.now() - started < this.pollTimeout) {
      ...
      if (status.status === 'ERROR' || status.status === 'CANCELED') {
        throw new ServiceUnavailableException(
          `LlamaParse job ${jobId} ended with status ${status.status}`,
        );
      }
      await new Promise((resolve) => setTimeout(resolve, this.pollInterval));
    }
    throw new ServiceUnavailableException(
      `LlamaParse job ${jobId} timed out after ${this.pollTimeout}ms`,
    );
  }
  ...
  async getMarkdown(jobId: string): Promise<string> {
    const res = await fetch(
      `${this.baseUrl}/api/parsing/job/${jobId}/result/markdown`,
      {
        headers: { Authorization: `Bearer ${this.apiKey}` },
      },
    );
    if (!res.ok) {
      throw new ServiceUnavailableException(
        `LlamaParse markdown fetch failed (${res.status})`,
      );
    }
