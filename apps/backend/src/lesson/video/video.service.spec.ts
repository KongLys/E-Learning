...
describe('VideoService', () => {
  ...
  describe('uploadVideo', () => {
    ...
    it('throws BadRequestException for non-video mimetype', async () => {
      ...
      await expect(
        service.uploadVideo('lesson-1', 'instructor-1', 'instructor', file),
    ...
    it('throws ForbiddenException when not the instructor', async () => {
      ...
      await expect(
        service.uploadVideo('lesson-1', 'user-2', 'instructor', validFile),
    ...
    it('uploads video and returns asset', async () => {
      ...
      mockStorage.uploadFile.mockResolvedValue(
        'http://minio/videos/lesson-1/uuid.mp4',
      );
      const asset = {
        id: 'asset-1',
        lessonId: 'lesson-1',
  ...
  describe('getSignedVideoUrl', () => {
    it('throws NotFoundException when lesson not found', async () => {
      mockPrisma.lesson.findUnique.mockResolvedValue(null);
      await expect(
        service.getSignedVideoUrl('lesson-1', 'user-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ForbiddenException when not enrolled and not preview', async () => {
      ...
      await expect(
        service.getSignedVideoUrl('lesson-1', 'student-1'),
      ).rejects.toThrow(ForbiddenException);
    });
