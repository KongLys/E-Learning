...
export default function PerformanceOverviewPage() {
  ...
  return (
    <div className="max-w-5xl space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-ink mb-1">Tổng quan</h1>
        <p className="text-sm text-muted">Phân tích hiệu quả giảng dạy của bạn</p>
      </div>

      {overview?.pendingQuestions > 0 && (
        <div className="flex items-center gap-3 bg-sun-soft border border-sun-deep rounded-xl px-5 py-4">
          <AlertTriangle size={18} className="text-sun-deep shrink-0" />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Đánh giá trung bình" value={overview?.avgRating ? <span className="inline-flex items-center gap-1">{overview.avgRating} <Star size={20} className="fill-sun text-sun" /></span> : '—'} />
      ...
      <div className="bg-surface-card rounded-card border border-hairline p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-ink">Doanh thu theo thời gian</h2>
      ...
      {ratingDistribution.some((r) => r.value > 0) && (
        <div className="bg-surface-card rounded-card border border-hairline p-6">
          <h2 className="text-base font-semibold text-ink mb-4">Phân bổ đánh giá</h2>
